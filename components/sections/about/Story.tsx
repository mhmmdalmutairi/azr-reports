    ...
    <section className="section-padding">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative">
          <div className="absolute top-0 bottom-0 right-1/2 md:right-1/2 w-px bg-gradient-to-b from-transparent via-[var(--azr)]/30 to-transparent" />
                  ...
                  <div className="flex-1 text-end md:text-start">
                    {i % 2 === 0 && (
                      <div className="md:text-end">
                        <span className="text-[var(--azr)] font-bold text-lg">{item.year}</span>
                        <h3 className="text-white font-semibold mt-1">{item.title[locale]}</h3>
                        <p className="text-white/50 text-sm mt-1">{item.desc[locale]}</p>
                      </div>
                    )}
                  </div>
                  <div className="relative z-10 w-4 h-4 rounded-full bg-[var(--azr)] border-4 border-[var(--ink)] flex-shrink-0" />
